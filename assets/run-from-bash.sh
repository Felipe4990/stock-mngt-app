...
JAVA_HOME="$PUBLIC_PATH/$PROJECT_NAME/jdk-21"; PATH=$PATH:$JAVA_HOME/bin; echo $PATH; nohup java \
    -jar $PUBLIC_PATH/$PROJECT_NAME/controle-estoque-app.jar \
        --spring.config.location=$PUBLIC_PATH/$PROJECT_NAME/resources/application.yaml > $PUBLIC_PATH/$PROJECT_NAME/log/output.log 2>&1 &

$(pwd)/assets/check-healthcheck.sh
